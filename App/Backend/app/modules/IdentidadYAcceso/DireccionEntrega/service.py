...
class DireccionEntregaService:
    ...
    @staticmethod
    def create(session: Session, data: DireccionEntregaCreate,
               usuario_id: int, es_admin: bool = False) -> DireccionEntrega:
        """
        ...
        """
        # Guard: only ADMIN users may create es_local addresses
        if data.es_local and not es_admin:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Solo administradores pueden crear direcciones de local",
            )
    ...
    def get_all(session: Session, usuario_id: int, es_admin: bool = False, incluir_locales: bool = False) -> list[DireccionEntrega]:
        ...
        with IdentidadYAccesoUnitOfWork(session) as uow:
            if es_admin:
                # ADMIN only manages company stores (locales). Never expose
                # clients' personal delivery addresses to the admin view.
                return uow.direcciones.get_locales()
            if incluir_locales:
    ...
    def update(
        ...
        with IdentidadYAccesoUnitOfWork(session) as uow:
            ...
            # Guard: only ADMIN users may set es_local
            if 'es_local' in values and values['es_local'] and not es_admin:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Solo administradores pueden modificar direcciones locales",
                )
