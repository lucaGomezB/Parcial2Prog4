...
class UsuarioCreate(BaseModel):
    ...
    created without a role. When used via POST /auth/register, this
    ...
    roles_codigos: Optional[str] = None
...
class UsuarioUpdateWithRoles(UsuarioUpdate):
    ...
    If roles_codigos is sent, the user's role is replaced with the
    new one (previous role is removed). If roles_codigos is omitted
    (None), the existing role remains unchanged.
    A user can only have ONE role.
    """
    roles_codigos: Optional[str] = None
