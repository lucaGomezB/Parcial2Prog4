...
/** Shortage for a single SHARED ingredient (aggregated across products). */
export interface ValidarStockIngrediente {
...
export interface ValidarStockResponse {
  ...
  /** Shared-ingredient shortages (aggregated across all requested items). */
  ingredientes: ValidarStockIngrediente[];
  /** Stock info for EVERY product in the request (always populated). */
  items: StockItemResponse[];
}

/** Stock information for a single product from the validar-stock endpoint. */
...
/** Input for the disponibilidad endpoint: current cart + product IDs to check. */
export interface DisponibilidadInput {
  carrito: ValidarStockDetalleInput[];
  productos: number[];
}

/** Addable quantity for a single product given the current cart. */
export interface ProductoLimitante {
  producto_id: number;
  nombre: string;
}
...
export interface ProductoDisponibilidad {
  producto_id: number;
  agregable: number;
  /** Other cart products that consume the ingredient constraining this one. */
  limitantes: ProductoLimitante[];
}
...
export const pedidosApi = {
  ...
  /**
   * Returns, for each requested product, how many more units can be added to
   * the cart without exceeding stock (including shared ingredients across
   * products). Read-only, used to mark products as unavailable in the menu.
   */
  disponibilidad: (data: DisponibilidadInput) =>
    apiFetch<DisponibilidadResponse>("/pedidos/disponibilidad", {
      method: "POST",
      body: JSON.stringify(data),
    }),
};
